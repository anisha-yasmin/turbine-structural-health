...
st.markdown("""
    ...
    /* Engineering Metric Cards Styling */
...
# --- ADVANCED TECHNICAL ADDITION: CUMULATIVE FATIGUE LIFE (MINER'S RULE) ---
# Calculates expected hours left based on current mechanical stress cycles
stiffness_ratio = I_damaged / I_healthy
if crack_depth == 0:
    hours_remaining = 175200.0  # 20 years baseline nominal life
else:
    # Damage accumulation factor scales exponentially with cross-sectional loss
...
if is_resonant:
    hours_remaining *= 0.1  # Resonance acceleration penalty decreases fatigue life tenfold

# --- SYSTEM METRICS PANEL ---
col1, col2, col3, col4 = st.columns(4)
with col1:
    st.metric(label="1st Mode Natural Freq (w1)", value=f"{omega_1:.3f} Hz")
with col2:
    st.metric(label="2nd Mode Natural Freq (w2)", value=f"{omega_2:.3f} Hz")
with col3:
    st.metric(label="Aeroelastic Diagnosis", value=resonance_status)
with col4:
    st.metric(label="Estimated Time to Failure", value=f"{int(hours_remaining):,} Hours")

st.markdown("---")

# --- VISUALIZATION 1: INTERACTIVE CAMPBELL DIAGRAM ---
st.subheader("Rotordynamic Campbell Diagram (Critical Speed Mapping)")
st.markdown("The intersections between the operational forcing frequencies (1P, 3P lines) and the structural natural frequencies (w1, w2) indicate mechanical resonance speeds.")

fig_campbell = go.Figure()

fig_campbell.add_trace(go.Scatter(x=rpm_range, y=freq_1P, name="1P Rotor Order (Unbalance Forcing)", line=dict(color="#1e3a8a", width=2, dash="dash")))
fig_campbell.add_trace(go.Scatter(x=rpm_range, y=freq_3P, name="3P Blade Passing Frequency (Aerodynamic Forcing)", line=dict(color="#dc2626", width=2.5, dash="dot")))
fig_campbell.add_trace(go.Scatter(x=rpm_range, y=np.ones_like(rpm_range) * omega_1, name="1st Flexural Mode (Flapwise w1)", line=dict(color="#16a34a", width=3)))
fig_campbell.add_trace(go.Scatter(x=rpm_range, y=np.ones_like(rpm_range) * omega_2, name="2nd Flexural Mode (Edgewise w2)", line=dict(color="#7c3aed", width=3)))
...
fig_fft = go.Figure()
fig_fft.add_trace(go.Scatter(x=freq_axis, y=total_signal, name="Sensor Telemetry Power Spectrum", fill='tozeroy', line=dict(color="#0284c7", width=2), fillcolor="rgba(2, 132, 199, 0.15)"))
fig_fft.update_layout(
    ...
    yaxis_title="Vibration Power Density (G2/Hz)",
    xaxis_gridcolor="#e2e8f0",
    yaxis_gridcolor="#e2e8f0"
)
